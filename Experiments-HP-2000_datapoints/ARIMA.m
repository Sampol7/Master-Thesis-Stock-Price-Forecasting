...
nu_t = fitted_t.nu;
disp('Degrees of Freedom t-dist res_arima:')
disp(nu_t);
disp('Kurtosis res_arima:')
disp(kurtosis(res_arima));
...
forecasting_error_logclose_ARIMA = test_logclose - forecasted_logclose_ARIMA;

%null hypothesis of no residual autocorrelation
[h_lbq,pValue_lbq,stat_lbq,cValue_lbq] = lbqtest(forecasting_error_logclose_ARIMA, Lags=num_lags);
disp(['LBQ p-value raw residuals: ', num2str(pValue_lbq)]);

% null hypothesis of no conditional heteroscedasticity
[h_arch, pValue_arch] = archtest(forecasting_error_logclose_ARIMA, Lags=5);
disp('ARCH test p-value raw Residuals: ');
disp(pValue_arch);

[pxx2, f2] = pspectrum(forecasting_error_logclose_ARIMA);
geometric_mean2 = exp(mean(log(pxx2)));
arithmetic_mean2 = mean(pxx2);
flatness2 = geometric_mean2 / arithmetic_mean2;
disp(['Flatness raw residuals:', num2str(flatness2)]);
