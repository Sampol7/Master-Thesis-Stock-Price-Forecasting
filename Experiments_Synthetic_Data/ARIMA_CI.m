...
for i=1:length(test_logclose)
    [forecasted_mean, ~, forecasted_variance] = forecast(fitARIMA, 1, logclose(i:length(train_logclose)+i-1));
    forecasted_logclose_ARIMA(i) = forecasted_mean;
...
forecast_errors = test_logclose - forecasted_logclose_ARIMA;
figure;
...
logCI_lower = forecasted_logclose_ARIMA - 1.96 * sqrt(forecasted_logclose_variance);
logCI_upper = forecasted_logclose_ARIMA + 1.96 * sqrt(forecasted_logclose_variance);
...
PICP_close = mean(in_CI_close) * 100;
disp(['Coverage of 95% CI on Close: ', num2str(PICP_close), '%']);
