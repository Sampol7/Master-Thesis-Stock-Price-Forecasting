...
%% Fit ARIMA-GARCH for various pA,qA,pG,qG
max_pqA = 5;
max_pqG = 2;
results_ARIMA_GARCH_p1q = ARIMA_GARCH_p1q(train_logclose, max_pqA, max_pqG);

%% Find best ARIMA-GARCH from all fits, based on aic, p-value, and correlation
best_pqAG = find_best_ARIMA_GARCH(train_logclose,results_ARIMA_GARCH_p1q);
...
disp(['Best ARIMA-GARCH model: ARIMA(', num2str(pA_best), ',1,', num2str(qA_best), ')-GARCH(', num2str(pG_best), ',', num2str(qG_best), ')']);
disp("pvalue_best:")
...
csvwrite('rawResiduals_ARIMA_GARCH.csv', res_arimagarch);
csvwrite('stanResiduals_ARIMA_GARCH.csv', stan_res_arimagarch);
...
xlabel('Date');
ylabel('Variance');
title('Moving Variance Residuals ARIMA-GARCH');
...
forecasted_logclose_variance = zeros(length(test_logclose),1);
for i=1:length(test_logclose)
    [forecasted_mean, ~, forecasted_variance] = forecast(fitARIMAGARCH, num_steps, logclose(i:length(train_logclose)+i-1));
    forecasted_logclose_ARIMAGARCH(i) = forecasted_mean;
    forecasted_logclose_variance(i) = forecasted_variance;
end

csvwrite('forecasted_logclose_ARIMA_GARCH.csv', forecasted_logclose_ARIMAGARCH);
...
disp('Best ARIMA-GARCH Forecast Log Close Error Metrics:');
disp(error_metrics_arima_garch_logclose);

disp('Best ARIMA-GARCH Forecast Log Close Directional Metrics:');
...
%% qq-plot forecast errors
forecast_errors = test_logclose - forecasted_logclose_ARIMAGARCH;
figure;
qqplot(forecast_errors);
xlabel('Theoretical Quantiles (Normal)');
ylabel('Sample Quantiles');
title('');
...
logCI_lower = forecasted_logclose_ARIMAGARCH - 1.96 * sqrt(forecasted_logclose_variance);
logCI_upper = forecasted_logclose_ARIMAGARCH + 1.96 * sqrt(forecasted_logclose_variance);

CI_lower_close = exp(logCI_lower);
CI_upper_close = exp(logCI_upper);

csvwrite('forecasted_close_ARIMA_GARCH.csv', forecasted_close_ARIMAGARCH);

%% Evaluate forecast close best arima
in_CI_close = (test_close >= CI_lower_close) & (test_close <= CI_upper_close);
coverage_close = mean(in_CI_close) * 100;
disp(['Coverage of 95% CI on Close: ', num2str(coverage_close), '%']);

interval_widths = CI_upper_close - CI_lower_close;
y_range = max(test_close) - min(test_close);
PINAW_close = mean(interval_widths) / y_range;

figure;
plot(test_close, 'b', 'DisplayName', 'Real Close');
hold on;
plot(forecasted_close_ARIMAGARCH, 'r', 'DisplayName', 'Forecasted Close');
plot(CI_lower_close, 'g--', 'DisplayName', '95% CI Lower');
...
disp('Best ARIMA-GARCH Forecast Close Error Metrics:');
disp(error_metrics_arima_garch_close);

disp('Best ARIMA-GARCH Forecast Close Directional Metrics:');
