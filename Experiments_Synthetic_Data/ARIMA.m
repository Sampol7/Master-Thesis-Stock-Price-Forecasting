...
%% Color plots pvalues and AIC
p = results_ARIMA_p1q(:,1);
q = results_ARIMA_p1q(:,2);
...
for i = 1:length(p)
    row = q(i) + 1; 
    col = p(i) + 1;
    pval_grid(row, col) = pval(i);
    aic_grid(row, col) = aic(i);
end
...
%% Function to add red boxes
function add_red_boxes(p, q, pval)
    hold on;
    for i = 1:length(p)
        if pval(i) < 0.05
            rectangle('Position', [p(i)-0.5, q(i)-0.5, 1, 1], 'EdgeColor', 'r', 'LineWidth', 2);
        end
    end
end
